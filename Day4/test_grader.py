...
#Define the answer key wich maps the question number to the correct answer
...
if len(cnts) > 0:
    ...
    for c in cnts:
        ...
        #IF our approximated contour has four points, then we can assume we have found the papaer
        if len(approx) == 4:
            docCnt = approx
            break
...
#Find countours in the thresholded image, then initialize the list of contours that correspond to questions
...
#Each question has 5 possible answers, to loop over the question in batchs of 5
for (q, i) in enumerate(np.arange(0, len(questionCnts), 5)):
    #Sort the contours for the current question from left to right, then initalize the 
    # index of the bubbled answer
    ...
    for (j, c) in enumerate(cnts):
        ...
        #Apply the mask to the thresholed image, then count the number of non-zero pixels in the bubble area
        mask = cv2.bitwise_and(thresh, thresh, mask=mask)
        total = cv2.countNonZero(mask)

        #If the current total has a larger number of total non-zero pixels, then we are examining the 
        # currently bubbled-in answer
